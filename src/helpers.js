...
var regexps = {
    ...
    crossDomainImg: /mmbiz.qpic.cn/i
};
...
var imgCrossDomainConfig = false;

module.exports.setImgCrossDomainConfig = function (cfg) {
    imgCrossDomainConfig = cfg;
};
...
function fixLinks(e) {
    ...
    function fixLink(link) {
        ...
        if (imgCrossDomainConfig && fixed.search(regexps.crossDomainImg) !== -1) {
            var fileName = md5(fixed) + path.extname(fixed);
            grabImgToPath(fixed, fileName);
            return imgCrossDomainConfig.imgUrlPrefix + fileName;
        }
        return fixed;
    }
    ...
    for (i = imgs.length - 1; i >= 0; --i) {
        var src = imgs[i].getAttribute('src') || imgs[i].getAttribute('data-src') || imgs[i].getAttribute('data-lazy-src');
        if (src) {
...
function grabImgToPath(url, fileName) {
    if (imgCrossDomainConfig && imgCrossDomainConfig.hasOwnProperty('imgStorePath')) {
        if (!fs.existsSync(imgCrossDomainConfig.imgStorePath)) {
            fs.mkdirSync(imgCrossDomainConfig.imgStorePath);
        }
        try {
            var writeStream = fs.createWriteStream(path.join(imgCrossDomainConfig.imgStorePath, fileName));
